...
(async function example() {
  ...
  try {
    ...
    for (let count = 0; count < 100; count++) {
      ...
      await driver
        .findElement(By.xpath("(//div/input)[1]"))
        .sendKeys(ImgsData[count].title);
      ...
      for (
        ...
        let card = await driver.findElement(
          By.xpath(`(//div[@data-os="-2"]/a)[${thumbCounter}]`)
        );
        if (await card.getAttribute("href")) {
          ...
          console.log(
            "thumbnail Img",
            thumbSrc[0] === "d" ? "True" : thumbSrc.slice(0, 50)
          );
          console.log(
            "HQ Img",
            imgSrc[0] === "h" ? "True" : thumbSrc.slice(0, 50)
          );
        }
      }
