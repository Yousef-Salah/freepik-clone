...
const SideBar = (props) => {
  ...
  return (
    <div
      id="sidebarr"
      className={`sidebar flex ${open ? 'tw-w-64 translatex100 ' : 'tw-w-0 translatex0'}`}
    >
      <div
        className={`${
      ...
      >
        ...
        <div className={`tw-flex tw-items-center tw-w-58 mb-5 ${!open && 'tw-scale-0'}`}>
                ...
                <div className="collapse" id={val.id}>
                  <span className="container-fluid ">
                    <div className={`row category-row sidebarrow `}>
                      {val.tag === 1
                        ? tags.map((tag, index) => (
                            <Tag title={tag} pic={val.icon1[index]} page={val.page} key={index} />
                          ))
                        : null}
                      {val.color === 1 ? (
                        colors.map((color, index) => (
                          <Colorr color={color} key={index} cancel={val.icon1[index]} />
                        ))
                      ) : val.sub === 1 ? (
                        subs.map((sub, index) => (
                          <>
                              ...
                              <span className="container ">
                                <div className={`row category-row  container`}>
                                  {sub.tags.map((tag, index) => (
                                    <Tag key={index} title={tag} page={val.page} />
                                  ))}
                                </div>
                              </span>
                            </div>
                        ...
                        <div className="form-check form-switch">
                          <input
                            ...
                            id={val.switchId}
                          />
                          <label className="form-check-label toggle-label">{val.par}</label>
                        </div>
                      ) : null}
                    </div>
                  </span>
