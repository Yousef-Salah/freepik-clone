import Categories from '../../../old_src/utils/Categories'
...
const DataGenerator = () => {
  ...
  let file_type = ['jpg', 'jpeg', 'png', 'psd', 'ai', 'eps', 'svg', 'gif', 'tiff', 'raw']
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    image.authour_image = `https://rextheme.com/wp-content/themes/RexTheme/images/testimonial-author-placeholder-img.png`
    ...
    image.publish_date = `${Math.floor(Math.random() * 20) + 10}-${
      Math.floor(Math.random() * 2) + 10
    }-${Math.floor(Math.random() * 8) + 2015}`
    image.choice = Math.random() < 0.5
    image.related_tags = [randomTitle, randomCategory, image.color, image.ethinicity, image.age]
